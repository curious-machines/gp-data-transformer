...
const grammar = {
    ...
    bnf: {
        ...
        statement: [
            [ "transforms", "$$ = { type: 'transform-sequence', transforms: $1 };" ],
        ...
        transform: [
            // NOTE: Ideally, we should be able to use the next commented out definition; however
            // this causes all sorts of shift/reduce errors due to the similarity of patterns and
            // expressions. If this can be worked out syntactically, a type pattern on its own
            // should simply return its return value and/or captures
            // [ "typePatterns", "$$ = { type: 'transform', patterns: $3, returnValue: null };" ],

            [ "expression <= typePatterns", "$$ = { type: 'transform', patterns: $3, returnValue: $1 };" ],
            [ "USE IDENTIFIER", "$$ = { type: 'transform-reference', name: $2 };" ]
        ],
