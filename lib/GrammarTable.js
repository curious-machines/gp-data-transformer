...
const grammar = {
    ...
    bnf: {
        program: [
            [ "statements", "return { type: 'program', statements: $1 };" ],
            [ "statements ;", "return { type: 'program', statements: $1 };" ]
        ],
        ...
        patternElement: [
            ...
            [ "pattern range", "$$ = { type: 'element', pattern: $1, range: $2 };" ],
            [ "( patternElements )", "$$ = { type: 'element-group', elements: $2, range: { type: 'range', start: 1, stop: 1 } };" ],
            [ "( patternElements ) range", "$$ = { type: 'element-group', elements: $2, range: $4 };" ]
        ],
        range: [
            [ "; integer .. integer", "$$ = { type: 'range', start: $2, stop: $4 };" ],
            [ "; .. integer", "$$ = { type: 'range', start: 0, stop: $3 };" ],
            [ "; integer ..", "$$ = { type: 'range', start: $2, stop: Infinity };" ],
            [ "; integer", "$$ = { type: 'range', start: $2, stop: $2 };" ],
            [ "*", "$$ = { type: 'range', start: 0, stop: Infinity };" ],
            [ "+", "$$ = { type: 'range', start: 1, stop: Infinity };" ],
            [ "?", "$$ = { type: 'range', start: 0, stop: 1 };" ]
        ],
        ...
        namedPatternProperty: [
            [ "patternProperty", passThrough ],
            [ "patternProperty AS IDENTIFIER", "$1.assignTo = $3; $$ = $1;" ]
        ],
        patternProperty: [
            [ "name : pattern", "$$ = { type: 'property', name: $1, pattern: $3 };" ],
            [ "name", "$$ = { type: 'property', name: $1, pattern: { type: 'pattern', patternType: 'any', value: null } };" ]
        ],
