...
export default class Transformer {
    ...
    executeTransform(transform, structure, symbolTable = {}) {
        ...
        if (transform.patterns !== null) {
            ...
            const value = this.executeGenerator(transform.returnValue, structure, symbolTable);
     ...
     * @returns {*}
     */
    executeGenerator(expression, structure, symbolTable) {
        const getNumbers = operation => {
            const left = this.executeGenerator(expression.left, structure, symbolTable);

            if (left !== FAILURE_VALUE && typeof left === "number") {
                const right = this.executeGenerator(expression.right, structure, symbolTable);
        ...
        switch (expression.type) {
            ...
            case "get-property": {
                const object = this.executeGenerator(expression.left, structure, symbolTable);
            ...
            case "map": {
                ...
                const values = this.executeGenerator(valueGenerator, structure, symbolTable);
            ...
            case "invoke": {
                const args = expression.args.reduce((accum, arg) => {
                    if (arg.type === "spread") {
                        ...
                        accum.push(this.executeGenerator(arg, structure, symbolTable));
                    }
            ...
            case "array":
                return expression.value.map(
                    elementExpression => this.executeGenerator(elementExpression, structure, symbolTable)
                );
            ...
            case "object": {
                ...
                for (const propertyExpression of expression.value) {
                    if (propertyExpression.type === "property") {
                        const value = this.executeGenerator(propertyExpression.value, structure, symbolTable);
