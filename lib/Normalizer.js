        ...
        switch (pattern.patternType) {
            ...
            case "array-pattern": {
                ...
                for (const element of pattern.value) {
                    const {pattern, range: {start, stop}, assignTo} = element;
                    let i;
                    ...
                    for (i = 0; i < stop; i++) {
                        ...
                        const value = (actualIndex < structure.length)
                            ? this.normalizePattern(pattern, structure[index + i])
                            : FAILURE_VALUE;
            ...
            case "object-pattern": {
                ...
                for (const property of pattern.value) {
                    const {name, pattern, assignTo} = property;
