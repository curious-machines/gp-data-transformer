...
export default class Normalizer {
    ...
    run(source, structure) {
        ...
        const reverseList = statements[0].expression;

        reverseList.reverse();

        for (const expression of reverseList) {
            const symbolTable = {};
            const result = this.normalizePattern(expression, structure, symbolTable);

            console.log(util.inspect(result));
            console.log(util.inspect(symbolTable));
